...
//        maybeAddCheckpointSpout(); // FIXME:AS1
    ...
    public <T extends State> BoltDeclarer setBolt(String id, IStatefulBolt<T> bolt, Number parallelism_hint) throws IllegalArgumentException {
        System.out.println("TEST: inside Topology builder code");
         Logger LOG = LoggerFactory.getLogger(TopologyBuilder.class);
        LOG.info("TEST: inside Topology builder code-1");
...
//                checkPointInputs.add(new GlobalStreamId(CHECKPOINT_COMPONENT_ID, CHECKPOINT_STREAM_ID));
                System.out.println("TEST_Adding_checkpointSpout_Stream");
