...
public class KeyedRoundRobinQueue<V> {
    ...
    public  int getSize(){
        int tot_size=0;
        System.out.println("TEST: Number of Queues:"+_queues.size());
        for(Queue<V> e: _queues.values()){
            tot_size+=e.size();
        }
        System.out.println("TEST:KeyedRoundRobinQueue Total tuples combining all Queues:"+tot_size);
        return tot_size;
    }
//
}
