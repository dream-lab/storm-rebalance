...
import static org.apache.storm.spout.CheckPointState.State.*;
...
//                nextState = recovering ? new CheckPointState(txid - 1, COMMITTED) : new CheckPointState(txid, COMMITTING);
                nextState = recovering ? this : new CheckPointState(txid, COMMITTING);
                if(!recovering)
                    LastPREPAREAck=true;
                break;
            case COMMITTING:
                nextState = new CheckPointState(txid, COMMITTED);
                System.out.println("LAST_commit_ack_message_setFLag_Tocreate_File");
                LastCheckpointAck = true;
                break;
            case COMMITTED:
