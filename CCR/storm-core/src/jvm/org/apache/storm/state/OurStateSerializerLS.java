 ...
 * the state.
 */
public class OurStateSerializerLS<T extends Collection> implements Serializer<T> {
    ...
    /**
     * Constructs a {@link OurStateSerializerLS} instance with the given list
     * of classes registered in kryo.
     *
     * @param classesToRegister the classes to register.
     */
    public OurStateSerializerLS(List<Class<?>> classesToRegister) {
        for (Class<?> klazz : classesToRegister) {
            kryo.get().register(klazz);
        }
    }

    public OurStateSerializerLS() {
...
//        return output.get().toBytes();
    ...
    public T deserialize(byte[] b) {
        ...
        CollectionSerializer serializer = new CollectionSerializer();
        return (T)serializer.read(kryo.get(),input,Collection.class);
