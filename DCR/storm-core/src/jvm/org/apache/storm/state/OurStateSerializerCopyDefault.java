 ...
 * the state.
 */
public class OurStateSerializerCopyDefault<T> implements Serializer<T> {
    ...
    /**
     * Constructs a {@link OurStateSerializerCopyDefault} instance with the given list
     * of classes registered in kryo.
     *
     * @param classesToRegister the classes to register.
     */
    public OurStateSerializerCopyDefault(List<Class<?>> classesToRegister) {
        for (Class<?> klazz : classesToRegister) {
            kryo.get().register(klazz);
        }
    }

    public OurStateSerializerCopyDefault() {
